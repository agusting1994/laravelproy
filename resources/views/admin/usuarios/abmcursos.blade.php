...
<h2 class="	text-primary">Cursos del usuario: {{ $user->nombre . " " . $user->apellido . " (". $user->email . ")" }}</h2>
<br>	
	<form id="demoform" action="/admin/usuarios/guardarcursos" method="post" onsubmit="return mifuncion()">
    <select multiple="multiple"  id="select-meal-type" size="30" name="duallistbox_demo1[]">
    	@foreach($cursos as $curso)
    		@foreach($user->cursos() as $cursou)
    			@if($user->cursos()->find($curso->id))
    			<div name="asdsasd">
